...
class ReviewGenerator(object):
  ...
  def attribute_encoder(self):
    ...
    with tf.variable_scope("attribute_encoder") as scope:
      ...
      attribute_output = tf.layers.dense(attr_concat, self.hidden_size * self.num_layers, 
  ...
  def _build_rnn_cell(self, num_units, forget_bias, num_layers, 
    ...
    cell_state = attribute_output
    hidden_state = attribute_output
    initial_state = tf.contrib.rnn.LSTMStateTuple(c=cell_state, h=hidden_state)

    print initial_state.c
    print initial_state.h

    if self.beam_width > 0:
      decoder_initial_state = tf.contrib.seq2seq.tile_batch(initial_state, self.beam_width)
    else:

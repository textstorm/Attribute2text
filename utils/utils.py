...
def get_iterator(dataset,
  ...
  tgt_sos_id = tf.cast(vocab_table.lookup(tf.constant("<s>")), tf.int32)
  tgt_eos_id = tf.cast(vocab_table.lookup(tf.constant("</s>")), tf.int32)

  src_tgt_dataset = tf.data.Dataset.from_tensor_slices(dataset)
